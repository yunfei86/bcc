...
if [ $# -eq 0 ];then
	...
	echo Example: "sh ../hi/getCoverage.sh ../../ref/Pfalciparum_PlasmoDB-8.1.gff 1000 1000 ../../data/GV_tophat.bed ../../ref/size2.txt"
	exit 1
fi
...
#1) find coverage
## up 
cat ${READS} | awk 'BEGIN{OFS="\t"} {if($6=="+"){print $1,$2,$3,$4,$5,"-"}else{print $1,$2,$3,$4,$5,"+"}}' > rev_reads.bed
coverageBed -d -s -a rev_reads.bed -b 1_ref/up.gff > 2_coverage/${READSNAME}_UP.coverage
## down
coverageBed -d -s -a rev_reads.bed -b 1_ref/down.gff > 2_coverage/${READSNAME}_DOWN.coverage
}
#2) tablize...
get_table(){
